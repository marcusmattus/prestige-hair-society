...
export async function POST(request:Request) {
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items:[{quantity:1,price_data:{currency:"gbp",unit_amount:service.deposit*100,product_data:{name:`Deposit hold · ${service.name}`,description:`${input.date} at ${input.time} · Balance ${pounds(service.price-service.deposit)} paid at the studio`}}}],
      payment_intent_data:{capture_method:"manual",description:`${reference} · ${service.name}`,metadata:{booking_reference:reference,booking_id:input.bookingId}},
      success_url:`${origin}/booking/success?session_id={CHECKOUT_SESSION_ID}`,
      cancel_url:`${origin}/#book`,
      metadata:{booking_reference:reference,booking_id:input.bookingId,service_id:service.id,service_name:service.name,service_price:String(service.price),deposit:String(service.deposit),appointment_date:input.date,appointment_time:input.time,first_name:input.firstName,last_name:input.lastName,email:input.email,phone:input.phone,notes:input.notes,email_updates:String(input.updates)},
      expires_at:Math.floor(Date.now()/1000)+30*60,
    },{idempotencyKey:`checkout-${input.bookingId}-${service.deposit}`});

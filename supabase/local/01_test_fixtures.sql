-- 01_test_fixtures.sql
-- LOCAL TEST ONLY. Never applied to a Supabase project.
--
-- The placeholder catalogue and the six fictional stylists that the SQL suites
-- in 02_constraint_tests.sql and 03_rls_tests.sql are written against. Those
-- tests need a stable shape -- two stylists eligible for the same service, a
-- service with a known price and buffer -- which the real catalogue does not
-- provide, since the salon has one stylist.
--
-- The REAL catalogue lives in supabase/catalogue.sql. Nothing in this file
-- describes the actual business.
--
--   ./scripts/db-reset.sh
--   psql -d prestige_test -f supabase/seed.sql
--   psql -d prestige_test -f supabase/local/01_test_fixtures.sql
